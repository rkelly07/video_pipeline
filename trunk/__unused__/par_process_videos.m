...
%cluster_profile = 'mikhail_cc2x256_1';
%num_workers = 150;
num_workers = 1;

% video_filenames = {'glassx2.mp4'};
% load(video_filenames{1});
% num_frames = floor(size(I,4)/num_workers)

% video_filename = 'my_glass1_gapless_mute.mp4';
video_filename = '../data/video1.ogv';
v = VideoStream(video_filename);
num_frames = v.NumFrames;

% disp('converting video file to .mat')
% video_filenames = {'KANE0302_20130712121923.AVI'};
% v = VideoReader(video_filenames{1});
% num_frames = floor(v.NumberOfFrames/num_workers);
...
%   end
% end
% save(video_filenames{1},'I');
% matfilename = [video_filenames{1} '.mat'];
% video_filenames = {matfilename};
% disp('Done!')

% video_filenames = { ...
%   '/shared/persisted/video1_raw.mat', ...
%   '/shared/persisted/video2_raw.mat', ...
%   '/shared/persisted/video3_raw1.mat', ...
%   '/shared/persisted/video3_raw2.mat' ...
%   };
...
% video_filenames={'/home/rosman/Documents/MIT/video_analysis/trunk/video1.ogv','/home/rosman/Documents/MIT/video_analysis/trunk/video2.ogv','/home/rosman/Documents/MIT/video_analysis/trunk/video3.ogv','/home/rosman/Documents/MIT/data/TelepresenceRobot_Video.ogv','/home/rosman/Documents/MIT/data/Glass2.ogv'};
%video_filenames={'/home/rosman/Documents/MIT/data/TelepresenceRobot_Video.ogv'};
% video_filenames={'/home/rosman/Documents/MIT/data/Glass2.ogv'};
caffe=[];
% TEST:
...
other_params = [];
other_params.descriptor_representatives = descriptor_representatives;
other_params.vq_preproc_mtx = vq_preproc_mtx;
...
% attached_files = cat(2,attached_files,dirrec(pwd,'.mp4'));
% attached_files = cat(2,attached_files,dirrec(pwd,'.avi'));

% attached_files = cat(2,attached_files,dirrec('../../matlab_distributed_coreset','.m'));
% attached_files = cat(2,attached_files,dirrec('../../analysis','.m'));
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% cluster = init_cluster(cluster_profile,'restart','NumWorkers',params.NumWorkers,'AttachedFiles',attached_files);
if 1%spmd
  pdisp('','SetVerbose',true)
  
  %   process_file(video_filenames{labindex},params,options,other_params);
  
  
  descriptor_representatives = other_params.descriptor_representatives;
  vq_preproc_mtx = other_params.vq_preproc_mtx;
  ...
  feature_coreset_leaf_size = 1000;
...
%   start_frame = (labindex-1)*num_frames+1
  start_frame = 1
 ...
 % video_stream = VideoStream(video_filenames{labindex},'StartFrame',start_frame);
%   video_stream = VideoStream(video_filenames{1},'StartFrame',start_frame);
...
%     [savefile_path,savefile_name,savefile_ext] = fileparts(video_filenames{1});
    [savefile_path,savefile_name,savefile_ext] = fileparts(video_filename);
    save_name=['data' filesep savefile_name '_results.mat'];
    save(save_name,'bags_of_words','processed_frame_idx');
...
% for f_i=1:numel(video_filenames)
%   %             try
%   process_file(video_filenames{f_i},labindex,params,options,feature_coreset_alg,semantic_coreset_alg,coreset_params);
%   %             catch
%   %             end
% end
% %
