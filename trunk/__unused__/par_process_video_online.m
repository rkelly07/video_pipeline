...
for n_worker=1:params.NumWorkers:numel(video_filenames)
      ...
      if options.GatherBagsOfWords
        if (params.use_caffe)
          ...
          if isfield(tracklets,'features') & ~isempty(tracklets.features)
            ...
            pdists = pdist2(tracklets.features,double(descriptor_representatives*vq_preproc_mtx));
  ...
  if options.Save
    for i = 1:numel(Composite)
      [savefile_path,savefile_name,savefile_ext] = fileparts(video_filename{labindex});
      save_name=['data/' savefile_name '_save_' num2str(i) '.mat'];
      disp(save_name);
      spmd_feature_coreset1=[];
      if options.GatherBagsOfWords
        spmd_feature_coreset1=spmd_feature_coreset{labindex};
      end
      % TODO: add here what ever variables you want to save
      save(save_name,'spmd_feature_coreset1');
    end
  end
  pdisp('Done!')
