function res=collect_ILVSRC_BOWs(res,data_dir,num_workers,worker_id,max_image,descriptor_aggregators)
directory_names=dir(data_dir);
image_ext='JPEG';
i=0;
save_global=false;
...
out_filename=[data_dir,filesep,'res_ILVSRC_',num2str(num_workers),'_',num2str(worker_id),'.mat'];
for i_dir=1:numel(directory_names)
    dir_name=[data_dir,filesep,directory_names(i_dir).name];
    ...
    if exist(dir_name,'dir')
        ...
        if (numel(xml_files)>0)
            for i_xml=1:numel(xml_files)
                ...
                img_spec=[dir_name,filesep,xml_name,'.',image_ext];
...
%                             end
                    ...
                    if (mod(i,1000)==0)|| i>=max_image
                        disp(i)
                        intermediate_descriptors_idx=1:intermediate_descriptors_cnt;
                        valid_idx=~(any(isnan(intermediate_descriptors)));
                        intermediate_descriptors_idx=intermediate_descriptors_idx(valid_idx(intermediate_descriptors_idx));
%                         intermediate_descriptors=intermediate_descriptors(:,valid_idx);
%                         intermediate_labels=intermediate_labels(:,valid_idx);
%                         intermediate_IDs=intermediate_IDs(:,valid_idx);
                        if save_bags_of_words
                            res.bows=[res.bows,intermediate_bows];
                            res.bows_class=[res.bows_class,intermediate_bows_class];
                        end
                        if (save_global_matrix)
                            res.descriptors=[res.descriptors,intermediate_descriptors(:,intermediate_descriptors_idx)];
                            res.labels=[res.labels,intermediate_labels(:,intermediate_descriptors_idx)];
                        end
                        % Here we can insert SGD, coresets, etc
                        if (~isempty(descriptor_aggregators) && ~isempty(intermediate_descriptors))
                            for aggregator_idx=1:length(descriptor_aggregators)
                                res=descriptor_aggregators{aggregator_idx}(res,intermediate_descriptors(:,intermediate_descriptors_idx),intermediate_labels(:,intermediate_descriptors_idx),intermediate_IDs(:,intermediate_descriptors_idx));
                            end
                        end
                        %                         intermediate_descriptors=[];
                        %                         intermediate_IDs=[];
