function process_video_lsda_wrapper( video_filename, coreset_results_filename,coreset_tree_data_filename )
...
if 1
    ...
    params.semantic_model=get_semantic_model(cfg,'lsda');
else
...
% video_filenames={'/scratch/rosman/loop4/guy/GOPR0022.MP4'};
%video_filenames={'/home/drl-leopard/LOCAL_DATA/videos/20150205_155823.mp4'};
% video_filenames={'/scratch/rosman/stata-2f-chairs.avi'};
...
%         save_suffix = ['_' num2str(num_frames) '_L' num2str(coreset_params.BOWCoresetLeafSize)];
        ...
        if exist('coreset_results_filename','var') && ~isempty(coreset_results_filename)
%             coreset_results_filename = [save_prefix 'coreset_results' num2str(file_no) save_suffix];
            disp(['Saving ' coreset_results_filename])
            save('-v7.3',[coreset_results_filename],'results');
            disp('Done!')
%             unix(['cp data/' coreset_results_filename '.mat data/coreset_results.mat']);
        end
        
        if exist('coreset_tree_data_filename','var') && ~isempty(coreset_tree_data_filename)
%             coreset_tree_data_filename = [save_prefix 'coreset_tree_data' num2str(file_no) save_suffix];
            ...
            save('-v7.3',[coreset_tree_data_filename],'coreset_tree_data');
            disp('Done!')
%             unix(['cp data/' coreset_tree_data_filename '.mat data/coreset_tree_data.mat']);
         end
