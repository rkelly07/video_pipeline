...
additional_options = []; 
if ispc
   additional_options = cat(2,additional_options, '-DWIN32 ');  
end
if DEBUG
    additional_options = cat(2,additional_options,'-g '); 
end
...
elseif ismac
    ...
    srcDep = cat(2,['-I',options.AuxFilepaths.OPENCV_INCLUDE,' -I/usr/include/eigen3 '],srcDep); 
end
...
elseif ismac
    lnkDepPath = cat(2,lnkDepPath, '-L/usr/local/lib ');
else
    lnkDepPath = cat(2,lnkDepPath, ['-L',options.AuxFilepaths.RTVID_LIB,' -L/usr/local/cuda/lib64 -L',options.AuxFilepaths.OPENCV_LIB]);
end
...
for i = 1:length(filesToCompile)
    ...
    cmd = sprintf('mex %s %s %s %s %s',additional_options,srcDep,lnkDepPath,lnkLib,filesToCompile{i});
    eval(cmd);
end
...
% ------------------------------------------------
% reformatted with stylefix.py on 2014/09/21 18:54
