...
params.UseTracking = false; % deprecated?
params.UseCaffe = false; % deprecated
...
params.WebcamNo = 0;
if 1
    params.semantic_words=true; % whether or not to use the semantic words as feature vectors for the coresets
    params.semantic_model=get_semantic_model(cfg,'rcnn'); % just the string - in the parameters, the initialization - just before the files loop / in process video, could be 'rcnn','lsda','places'
else
