function [v,dets]=detect10k_vector(rcnn_model, rcnn_feat, imname)
th = tic;
% Read image
if (ischar(imname))
im = imread(imname);
...
%    rcnn_model.classes);
v=zeros(num_categories,1);
for i = 1:size(top_boxes,1)
    v(cats_ids(i))=v(cats_ids(i))+1;
end
dets=[cats_ids(:)+200,top_boxes]; %1..200 are reserved for original rcnn/ilsvrc13 200 categories
fprintf('\n Final time: %.3fs\n', toc(th));
% Show detections
