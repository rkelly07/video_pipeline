classdef Stream < handle
    ...
    methods
        ...
        function addPointSetBatches(obj, P)
            ...
            for i = 1:ceil(P.n/obj.leafSize)
                pdisp(repmat('- ',1,20));
                xa = obj.leafSize*(i-1)+1;
                xb = obj.leafSize*i;
                
                % boundary condition fix
                xb = min(xb,P.n);
                
                pdisp(['Adding points ' num2str(xa) '-' num2str(xb)])
                Q = P.subset(xa:xb);
                obj.addLeaf(Q);
            end
            pdisp(repmat('- ',1,20));
            ...
            %   %                 Q=P.subset(1:obj.leafSize);
            %   obj.addLeaf(P.subset(1:obj.leafSize));
            %   %                 M=;
            %   %MM=PointFunctionSet(M);
            %   %MM.size
        ...
        function allC = getUnifiedCoreset(obj)
            ...
            s = Stack(obj.stack);
            % todo: init with the first coreset instead this line
            %allC=feval(class(s.top().coreset));
            isFirst=true;
            while not(s.isEmpty())
                C=s.pop().coreset;
                if isFirst
                    ...
                    %allC.merge(C);
                    allC = obj.coresetAlg.mergedCoreset(allC,C);
                end
            end % while
            if isempty(obj.lastLeaf)
                nLeaf=0;
            ...
            if nLeaf>0
                ...
                allC = obj.coresetAlg.mergedCoreset(allC,obj.lastLeaf);
            end
    ...
    methods (Static)
        function testKMedianStream(n, d, k, iterations, leafSize)
            ...
            % calg.setParameters(obj, kk, b, d, tt, cType)
        ...
        function test2(n,d,j,iterations, leafSize)
            a = LowDimOriginCoresetAlg();
            a.j=j;
            a.originSampleSize=10;
            a.lowDimSampleSize=20;
